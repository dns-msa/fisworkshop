...
aws ssm send-command \
	--targets '[{"Key":"tag:Name","Values":["ChaosProductStack/productAsg"]}]' \
	--document-name "AWS-RunShellScript" \
	--max-concurrency 1 \
	--parameters '{"commands":["#!/bin/bash","pkill -f java && sleep 5", "cd /root/product && sudo sh start.sh 1>/dev/null 2>&1 &", "sleep 30"]}'
    #--output text
